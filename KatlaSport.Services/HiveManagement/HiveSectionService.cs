...
namespace KatlaSport.Services.HiveManagement
{
    ...
    public class HiveSectionService : IHiveSectionService
    {
        ...
        {
            var dbSections = await _context.Sections.Where(h => h.Code == createRequest.Code).ToArrayAsync().ConfigureAwait(false);
        ...
        {
            var dbSections = await _context.Sections.Where(p => p.Code == updateRequest.Code && p.Id != sectionId).ToArrayAsync().ConfigureAwait(false);
            if (dbSections.Length > 0)
            {
                throw new RequestedResourceHasConflictException("code");
            }

            dbSections = await _context.Sections.Where(p => p.Id == sectionId).ToArrayAsync().ConfigureAwait(false);
            if (dbSections.Length == 0)
            {
                throw new RequestedResourceNotFoundException();
            }
        ...
        {
            var dbSections = await _context.Sections.Where(p => p.Id == sectionId).ToArrayAsync().ConfigureAwait(false);
            if (dbSections.Length == 0)
            {
                throw new RequestedResourceNotFoundException();
            }
